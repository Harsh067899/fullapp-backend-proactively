...
const PatientDashBoard = () => {
	...
	useEffect(() => {
		...
		if (!jwtToken) {
			...
			toast.error('Access denied. Please login as a Listener!');
			setTimeout(() => {
				return navigator('/patient-login');
			}, 1500);
		} else {
					...
					<div className='flex flex-col justify-center w-full md:w-[calc(100%-24rem)]'>
						<div className='pt-24 m-auto pb-1 border-red-600 flex justify-around items-center md:w-[calc(100%-10rem)]'>
							{window.location.pathname !== '/patient-dashboard/pending' && ( // the dropdown below allows patients to change their location and view doctors in that location
								<select id='countries' className='block m-2 md:min-w-0 rounded-md p-2 border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6 h-8' onChange={handleLocationChange} value={location}>
							...
							{window.location.pathname === '/patient-dashboard/consultations' ? (
								<Link to='/patient-dashboard' className='m-2 flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500'>
									All Speakers
								</Link>
							) : (
								<Link to='/patient-dashboard/consultations' className='m-2 flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500'>
									Your Consultations
								</Link>
							)}
						</div>
						{/* Needs styling... */}
						<h2 className='m-auto font-bold my-2 text-xl uppercase'>{window.location.pathname === '/patient-dashboard/consultations' ? 'Pending Consultations' : 'List of available Speakers'}</h2>
